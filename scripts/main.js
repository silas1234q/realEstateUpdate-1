...
let listBoxHtml='';
    ...
    let homelistings =[listingsInfo[0],listingsInfo[1]];

    // console.log(homelistings);

    homelistings.forEach(info=>{
        listBoxHtml +=`<div class="list-box">
    <div class="list-box-container">
    <img src="${info.image}" alt="" data-aos="fade-left" data-aos-duration="2000">
    ...
    <button  data-aos="fade-in" data-aos-duration="2000" data-name="${info.id}" >view more</button>
    </div>`
    });
     function updateDom(listBoxHtml){
        document.querySelector('.list-container').innerHTML = listBoxHtml;
    }
    updateDom(listBoxHtml)
    AOS.init();
    ...
    listBox.forEach(box=>{
        ...
        Array.from(btns).forEach(btn=>{
            ...
            btn.addEventListener('click',()=>{
                ...
                fulllistingArrs.pop()
                fulllistingArrs.push(targetView); 
                let newFullListingArr = fulllistingArrs;
                let fullListings ='';
                fulllistingArrs.forEach(item=>{
                    let {image,beedrooms,Price,squarefeet} = item;
...
let houseSideImages = document.querySelector('.house-info-wrapper');
